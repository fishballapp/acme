...
- Custom resolvers should return `[]` when a record is not found yet, and only
  throw for actual resolver failures.
...
- DoH resolvers return `[]` for `NXDOMAIN`, but throw for other DNS failure
  statuses and HTTP/network failures.
...
Provider-grouped constants are also available when you want to select resolver
targets explicitly:
